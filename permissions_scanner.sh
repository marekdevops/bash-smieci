...
def get_permissions(path):
    """Zwraca uprawnienia w formacie liczbowym i czytelnym dla człowieka"""
    try:
        file_stat = os.stat(path)
        mode = file_stat.st_mode
        
        # Konwersja uprawnień na format rwxrwxrwx
        perms_symbolic = stat.filemode(mode)
        
        # Konwersja na format oktalny (liczbowy)
        perms_octal = oct(stat.S_IMODE(mode))[2:]  # Usuwa '0o' z początku
        ...
        return f"{perms_octal} {perms_symbolic} {owner}:{group} {path}"
...
def scan_directories_only(root_path="/", output_file="directories_permissions.txt"):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("Format: OCTAL SYMBOLIC OWNER:GROUP PATH\n")
        f.write("=" * 60 + "\n\n")
